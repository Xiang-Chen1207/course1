        ...
        for step, batch in pbar:
            ...
            with my_context():
                with torch.amp.autocast("cuda"):
                    ...
                    # Apply weights
                    rec_weight = getattr(args, 'rec_loss_weight', 1.0)
                    reg_weight = getattr(args, 'reg_loss_weight', 1.0)
                    
                    loss = (loss_rec + loss_rec_sym) * rec_weight
                    
                    if features is not None and reg_output is not None:
                        reg_loss = F.mse_loss(reg_output, features)
