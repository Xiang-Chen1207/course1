...
class NeuralTransformerForMaskedEEGModeling(nn.Module):
    def __init__(self, EEG_size=1600, patch_size=200, in_chans=1, out_chans=8, vocab_size=8192, embed_dim=200, depth=12,
                 ...
                 use_abs_pos_emb=True, use_rel_pos_bias=False, use_shared_rel_pos_bias=False, init_std=0.02, num_regression_features=0):
        super().__init__()
        self.num_features = self.embed_dim = embed_dim  # num_features for consistency with other models
        self.num_regression_features = num_regression_features
        ...
        if use_abs_pos_emb:
            self.pos_embed = nn.Parameter(torch.zeros(1, 140 + 1, embed_dim))
        else:
            self.pos_embed = None
        ...
        if self.num_regression_features > 0:
            self.reg_head = nn.Linear(embed_dim, num_regression_features)
            trunc_normal_(self.reg_head.weight, std=self.init_std)
        else:
            self.reg_head = None
    ...
    def forward(self, x, input_chans=None, bool_masked_pos=None, return_all_tokens=False, return_patch_tokens=False, return_all_patch_tokens=False):
        ...
        if return_all_tokens:
            return self.lm_head(x), reg_output
        else:
            # return the masked tokens
            return self.lm_head(x[bool_masked_pos]), reg_output
...
class NeuralTransformerForMEM(nn.Module):
    def __init__(self, EEG_size=1600, patch_size=200, in_chans=1, out_chans=8, vocab_size=8192, embed_dim=200, depth=12,
                 ...
                 drop_path_rate=0., norm_layer=None, init_values=None, attn_head_dim=None,
                 use_abs_pos_emb=True, use_rel_pos_bias=False, use_shared_rel_pos_bias=False, init_std=0.02, num_regression_features=0, **kwargs):
        super().__init__()
        self.patch_size = patch_size
        self.student = NeuralTransformerForMaskedEEGModeling(EEG_size, patch_size, in_chans, out_chans, vocab_size, embed_dim, depth,
                 ...
                 use_abs_pos_emb, use_rel_pos_bias, use_shared_rel_pos_bias, init_std, num_regression_features=num_regression_features)
    ...
    def forward(self, x, input_chans=None, bool_masked_pos=None):
        x_masked, reg_output = self.student(x, input_chans, bool_masked_pos, return_all_patch_tokens=True)
        x_masked_no_cls = x_masked[:, 1:]
        x_rec = self.lm_head(x_masked_no_cls[bool_masked_pos])

        #symetric
        x_masked_sym, _ = self.student(x, input_chans, ~bool_masked_pos, return_all_patch_tokens=True)
        x_masked_no_cls_sym = x_masked_sym[:, 1:]
        x_rec_sym = self.lm_head(x_masked_no_cls_sym[~bool_masked_pos])

        return x_rec, x_rec_sym, reg_output
