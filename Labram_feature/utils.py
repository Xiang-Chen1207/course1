...
class TUABLoader(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        file_path, trial_key, segment_key, label = self.samples[index]
        try:
            ...
            elif 'chOrder' in f[trial_key][segment_key]['eeg'].attrs:
            ...
            print(f"Error loading {file_path} {trial_key} {segment_key}: {e}")
            # Return a dummy or handle error. 
            # Ideally we shouldn't fail. returning zeros for now to prevent crash
            return torch.zeros((21, 2000)), 0
...
def prepare_TUAB_dataset(root):
    ...
    for file_path in files:
        try:
            with h5py.File(file_path, 'r') as f:
                ...
                if hasattr(dataset_name, 'decode'):
                    dataset_name = dataset_name.decode('utf-8')

                category_list = f.attrs.get('category_list', ['normal', 'abnormal'])
                if hasattr(category_list[0], 'decode'):
                    categories = [c.decode('utf-8') for c in category_list]
                else:
                    categories = [str(c) for c in category_list]
                ...
                for trial_key in f.keys():
                    ...
                    trial = f[trial_key]
                    task_name = trial.attrs['task_name']
                    if hasattr(task_name, 'decode'):
                        task_name = task_name.decode('utf-8')
                    
                    label = cat_map.get(task_name)
                    if label is None:
                        continue
                        
                    for segment_key in trial.keys():
                        if not segment_key.startswith('segment'):
                            continue
                        
                        sample = (file_path, trial_key, segment_key, label)
                        
                        if 'train' in dataset_name:
                            train_samples.append(sample)
                        elif 'eval' in dataset_name or 'val' in dataset_name:
    ...
    print(f"Found {len(train_samples)} train, {len(val_samples)} val, {len(test_samples)} test samples.")
    
    if len(test_samples) == 0 and len(val_samples) > 0:
        print("No test set found, using validation set as test set.")
        test_samples = val_samples
    
    train_dataset = TUABLoader(train_samples)
    test_dataset = TUABLoader(test_samples)
    val_dataset = TUABLoader(val_samples)
...
def prepare_custom_dataset(root):
    # set random seed
    seed = 12345
    np.random.seed(seed)
    ...
    def process_files(file_list, sample_list):
                    ...
                    for trial_key in f.keys():
                        ...
                        for seg_key in trial.keys():
                            ...
                            if 'eeg' in seg:
                                ...
                                sample_list.append((file_path, trial_key, seg_key, int(lbl)))
            except Exception as e:
                print(f"Error reading {file_path}: {e}")

    print("Processing train files...")
    process_files(train_files, train_samples)
    print("Processing test files...")
    process_files(test_files, test_samples)
    
    print(f"Found {len(train_samples)} train, {len(test_samples)} test samples.")
    
    train_dataset = TUABLoader(train_samples)
    test_dataset = TUABLoader(test_samples)
    val_dataset = TUABLoader(test_samples) # Use test as val
